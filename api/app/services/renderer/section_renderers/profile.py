...
# Brand icon paths sourced from simpleicons.org (CC0). Keys must match
# `SocialIconKey` in `web/src/lib/sections/socialIcons.tsx`; the React and
# Python renderers are kept in sync by hand. The "globe" entry is the
...
def render_profile(data: dict, context: dict | None = None) -> str:
    ...
    row_gap = instance_style.get("row_gap") or css_vars.get("--row-gap", "8px")
    parts = []
    if data.get("photo_url"):
        parts.append(
            f'<img src="{data["photo_url"]}" alt="" '
            f'style="width:80px;height:80px;border-radius:9999px;object-fit:cover;'
            f'border:2px solid var(--accent, transparent);" />'
        )
    # Render as a plain <div> rather than an <h2> — heading elements carry
    # UA-default line-height that visually inflates the wrapper gap above
    # the next row, even with `margin:0` set inline.
    parts.append(
        f'<div class="f-name" style="margin:0;">'
        f'{esc(data.get("name")) or "Your Name"}</div>'
    )
    parts.append(
    ...
    if data.get("email"):
        if data.get("email_link", True) is not False:
            contact_items.append(
                f'<a href="mailto:{esc_attr(data["email"])}" class="f-contact f-email">'
                f'{esc(data["email"])}</a>'
            )
        else:
            contact_items.append(
                f'<span class="f-contact f-email">{esc(data["email"])}</span>'
            )
    if data.get("phone"):
        contact_items.append(
            f'<span class="f-contact f-phone">{esc(data["phone"])}</span>'
        )
    if data.get("location"):
        contact_items.append(
            f'<span class="f-contact f-location">{esc(data["location"])}</span>'
        )
    if data.get("site_url"):
        ...
        contact_items.append(
            f'<a href="{site_href}" class="f-contact f-site" '
            f'target="_blank" rel="noopener noreferrer">'
            f'{esc(site_text)}</a>'
        )
    if contact_items:
        # Each separator carries the f-contact-sep class so the divider-color
        # stylesheet rule keeps matching even though the wrapper no longer
        # inherits `f-contact`.
        sep = '<span class="f-contact-sep" style="margin:0 6px;color:var(--divider, #d1d5db)">·</span>'
    ...
    if social_links:
        ...
        for link in social_links:
            ...
            icon_key = link.get("icon") or "globe"
            svg_path = _SOCIAL_ICON_SVG.get(icon_key, _SOCIAL_ICON_SVG["globe"])
            href = esc_attr(normalize_url_scheme(url))
            items.append(
                f'<a href="{href}" class="f-social-link" target="_blank" '
                f'rel="noopener noreferrer" '
                f'style="display:inline-flex;align-items:center;gap:4px;text-decoration:none;">'
        ...
        if items:
            parts.append(
                ...
                'justify-content:center;gap:12px 12px;font-size:0.75rem;">'
                + "".join(items)
                + "</div>"
            )
    if data.get("summary"):
        parts.append(
            f'<p class="f-summary" style="margin:0;">'
    ...
    body = f'<div style="display:flex;flex-direction:column;gap:{row_gap};">{"".join(parts)}</div>'
    return f'<div class="profile-section"{wrapper_attr}>{body}</div>'
