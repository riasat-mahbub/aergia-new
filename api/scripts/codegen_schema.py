...
issues. A small in-tree generator has no new deps and emits the exact output
the frontend wants.
...
- A header comment carries the SHA-256 of the source file so CI can detect
  drift.
...
def main() -> int:
    ...
    if args.check:
        ...
        if existing != output:
            sys.stderr.write(f"Drift: {args.out} would change. Run `npm run codegen` to update.\n")
            return 1
        return 0
